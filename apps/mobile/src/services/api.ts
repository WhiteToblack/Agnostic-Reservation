...
export const signUp = (payload: SignInPayload & { fullName: string; preferredTheme: string }) =>
  client.post('/auth/signup', { tenantId: payload.tenantId, email: payload.email, password: payload.password, fullName: payload.fullName, preferredTheme: payload.preferredTheme }).then((res) => res.data);

export const fetchDashboard = (tenantId: string, roleId: string) => client.get('/dashboard', { params: { tenantId, roleId } }).then((res) => res.data);
...
export const fetchParameters = (tenantId: string) => client.get('/admin/parameters', { params: { tenantId } }).then((res) => res.data);
